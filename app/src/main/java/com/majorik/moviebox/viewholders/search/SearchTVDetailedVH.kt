...
class SearchTVDetailedVH(val parent: View) : RecyclerView.ViewHolder(parent) {
    @UseExperimental(ExperimentalStdlibApi::class)
    fun bindTo(item: TV?) {
        item?.let {
            itemView.card_image.displayImageWithCenterCrop(UrlConstants.TMDB_BACKDROP_SIZE_1280 + it.backdropPath)
            itemView.card_title.text = it.name

            if (!it.firstAirDate.isNullOrEmpty()) {
                itemView.card_release_date.text = it.firstAirDate!!.toDate().yearInt.toString()
            }
